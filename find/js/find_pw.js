...
async function findPw() {
  ...
  const idInput = document.querySelector(".find-form #user-id");
  ...
  findBtn.addEventListener("click", () => {
    ...
    switch (checkedIdx) {
      case(0): 
        findPwWithEmail(id, name);
        break;
      case(1): 
        findPwWithCall(id, name);
        break;
      default:
        break;
    }
  ...
  //이메일로 찾기
  function findPwWithEmail(id, name) {
  ...
  //휴대폰 번호로 찾기
  function findPwWithCall(id, name) {
    ...
    //일치하는 회원 찾기
    let user = users.find(user => {
      return user.id === id && user.name === name && user.call === call;
    });
...
//임시 비밀번호 표시
function displayPw(user) {
  const findForm = document.querySelector(".find-form");
  const resultBox = document.querySelector(".result-box");
  const pwTxt = resultBox.querySelector(".result-txt span");
  ...
  //알파벳, 숫자, 특수문자가 모두 들어간 12글자 임시 비밀번호 제작
  while (contain < 3) {
    newPw = "";
    for (let i=0; i<12; i++) {
      newPw += letter[Math.floor(Math.random() * letter.length)];
    }

    contain = 0;
    contain += /[a-zA-Z]+/.test(newPw);
    contain += /\d+/.test(newPw);
    contain += /[~!@#$%^&*()_\-+=]/.test(newPw);
  }

  pwTxt.textContent = newPw;
  user.pw = newPw;
}
